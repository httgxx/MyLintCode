 ...
 * @Idea 求最短路径=>BFS T=O(n*2^n) S=O(n*2^n)
...
public:
    ...
    int shortestPathLength(vector<vector<int>> &graph) {
        ...
        int steps = 0;
        int final = (1 << n) - 1;      // 目标状态: 所有n个顶点都被访问过
        unordered_set<int> visited;    // 已访问过的状态(编码)
        queue<pair<int, int>> q;       // <当前结点,当前状态>
        for (int i = 0; i < n; ++i) {  // 初始化: 所有顶点入队,即可从任意顶点出发
            q.push({i, 1 << i});        
        }
        ...
        while (!q.empty()) {    // BFS
            ...
            while (cnt--) {
                ...
                if (state == final) { return steps; }       // 结束否
                int stateCode = (node << 16) | state;       // 状态编码
