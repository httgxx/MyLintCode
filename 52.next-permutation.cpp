 ...
 * 2.从左向右扫那个最长降序子串,直到找到比字符[i]大的最小/最右边元素[j],将[i]与[j]交换 //[2,4,3,1]
 * 3.将交换后不再降序的那个子串按从小到大重新排列,以保证得到位置i上变成[j]后的最小排列  //[2,1,3,4]
 * 
 * 考虑有重复元素的情况 [3,4,4,2,2]=>[4,2,2,3,4]
 * 首先找到需要增大的数字,即从右往左的第1个变小的数字3(须跳过与第2个4相等的第1个4继续向左找)
 * 然后找到可以替换3的数字,即从右往左的最小的比3大的数字(第2个4)
 * 最后重新从小到大排列替换后的原最长降序子串4,3,2,2 => 2,2,3,4得到最终结果[4,2,2,3,4]
 */
class Solution {
public:
    /**
