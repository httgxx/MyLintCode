 ...
 * dp[i][j]=min{dp[i-1][k]|kj不组成01 + 1|a[i-1]!=j即第i位需要翻转1次}
 * 初始 dp[0][0]=dp[0][1]=0
...
public:
    ...
    int flipDigit(const vector<int> &nums) {
        ...
        dp[0][0] = dp[0][1] = 0;  // 初始化
        for (int i = 1; i <= n; ++i) {  // 坑:前i位指a[0]~a[i-1],从i=1开始
            for (int j = 0; j <= 1; ++j) {  // 当前位可能值0,1
                // 对前位的每种可能值,看能否找到更小的翻转次数
                for (int k = 0; k <= 1; ++k) {  // 前位可能值0,1
