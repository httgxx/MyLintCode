 ...
 * 1.p尾非'*': s尾同p尾,条件:i>0且(p[j-1]=='?'或s[i-1]==p[j-1]): dp[i][j]=dp[i-1][j-1]
 * 2.p尾是'*': s尾配0或多个p尾 
 *   条件1:s尾配0个p尾前 => 跳过p尾不用配: dp[i][j]=dp[i-1][j]
 *   条件2:s尾配多个p尾前中的最后一个 => s尾配掉1个p尾后,s前一尾配0或多个p尾: dp[i][j]=dp[i][j-1]
 * 
 * 大坑:初始dp时千万小心:
 *      dp[0][0]=true    //前0个字符配空串
 *      dp[i>0][0]=false //前多个字符不配空串
 * 大坑:不能认定dp[0][j]=false(前0个字符不配非空串) - 错!!!因为p可以为*,**,***,...都可配前i个字符
 *      dp[i>0][j>0]= if(p[j-1]=='*'): (p[j-1]=='?' || s[i-1]==p[j-1]) && dp[i-1][j-1]
 *                 else: dp[i][j-1] || dp[i-1][j]   //配0个p尾 或 配多个p尾中的最后一个
 *      返回 dp[m][n]
 * 大坑:不能认定dp[0][j>0]=false(前0个字符不配非空串) - 错!!!因为p可以为*,**,***,...都可配前i个字符
 *      所以dp[0][j>0]也得由公式计算!!!
 * */
class Solution {
public:
