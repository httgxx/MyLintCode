...
public:
    ...
    vector<string> topKFrequentWords(vector<string> &words, int k) {
        ...
        for (int i = res.size() - 1; i >= 0; --i) {  // 每次输出最小堆顶故从小到大输出,需反序从数组尾往前写
            res[i] = q.top().first; q.pop();
        }
        return res;
    }

    // S2: map + 桶Bucket<set> // T=O(nlogn)
    vector<string> topKFrequentWords2(vector<string> &words, int k) {
        vector<string> res;
        unordered_map<string, int> freq;
        vector<set<string>> buckets(words.size() + 1, set<string>());  // size+1 set自动按字母升序排序
        for (string word : words) { ++freq[word]; } // 统计频率
        for (auto a : freq) { buckets[a.second].insert(a.first); }  //入桶:按频率找桶 set自动排序O(logn)
