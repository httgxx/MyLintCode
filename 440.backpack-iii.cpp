 ...
 * for i = 1 ~ m  // 由小到大的目标和
 *     for j = 0 ~ n-1  // 物品数组坐标
 *         if (j>=a[i]) dp[j] = max(dp[j], v[i]+dp[j-a[i]])
 * 返回 dp[m]
 *
 * 坑 计数型初始化一定注意,如此题中dp[0]=1而不是0
 * 坑 完全背包:数组坐标必须正循环(0~n-1),目标和必须正循环(0~m)
 *    必须外循环目标和,内循环数组坐标且加条件if(i>=a[j])
 */
class Solution {
public:
    /**
    ...
    int backPackIII(vector<int> &A, vector<int> &V, int m) {
        ...
        // dp[0][i]=0, dp[0][j]=0
        for (int i = 1; i <= m; ++i) {  // 坑: 必须正序!!!因为要用小index的新值来算大index的新值
            for (int j = 0; j < A.size(); ++j) {  // 坑: 体积存在A[i]而不是V[i] 看清条件
                if (i >= A[j])
                    f[i] = max(f[i], f[i - A[j]] + V[j]);  // 不选A[i]时=f[j]不占容积不算价值
    ...
    int backPackIII2(vector<int> &A, vector<int> &V, int m) {
        vector<int> f(m + 1, 0);  // 坑:f(m+1,0)针对从小到大的背包容量i,而不是f(n+1,0)前i个物品
        // dp[0][i]=0, dp[0][j]=0
