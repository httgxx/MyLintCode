...
public:
     ...
     */
    // S1: hashtable记字母频率
    // 窗口内满足条件hashtable.size()<=2, 左移使得 先右边界+1再左边界+n
    // T= O(n) S=O(1)
    int lengthOfLongestSubstringKDistinct1(string &s, int k) {
        ...
        for (int right = 0; right < s.size(); ++right) {        // 右边界+1扩大窗口
            ++m[s[right]];                                      // 频率+1, 若m不含会自动新建s[i]
            while (m.size() > k) {                              // 左边界不断+1缩小窗口直到满足条件
                if (--m[s[left]] == 0) { m.erase(s[left]); }    // 频率-1, 若频率减为0则删除s[i]
                ++left;
            }
    ...
    int lengthOfLongestSubstringKDistinct(string &s, int k) {
        ...
        for (int right = 0; right < s.size(); ++right) {
            m[s[right]] = right;                                    // 更新s[right]字母最后出现的位置
            while (m.size() > k) {                              // 左边界不断+1缩小窗口直到满足条件
                if (m[s[left]] == left) { m.erase(s[left]); }   // 左边界上的字符最后出现在位置,出窗后需从m删除
                ++left;
            }
            res = max(res, right - left + 1);                   // 更新最大窗口大小 // 坑:勿忘+1!!!
        }
