 ...
 * @Ideas 
 * DP T=O(n) S=O(n)->O(1)
 * dp[i]表示s[0~i-1]的解码方式总数 =>dp.size=n+1, 
 *      i可以看作当前处理的子串长度
 * for i=2~n: 
 *     dp[i] = decodeOne(s[i-1])*dp[i-1]        //单独解码[i-1]
 *           + decodeTwo(s[i-2],s[i-1])*dp[i-2] //组合解码[i-2][i-1]
 *     dp[i] %= 1e9+7
 ...
 * 1~9: 1种 (数字本身)
 * 
 * 组合解码 decodeTwo(pre, cur)
 *   **: 15种(11~19,21~26) //为什么不是10~26??
 *   *x: 1种(x=7~9时)或2种(x=1~6时)
 *   x*: 9种(x=1时11~19)或6种(x=2时21~26)或0种(x=0,3~9时) //为什么不是10~19,20~26??
 *   xy: 1种(xy=11~26时)或0种(xy<10或>26时)
 * 
 * 降维: dp[i-1]=>dp[1], dp[i-2]=>dp[0]
 * for i=2~n: 
 *     long dp_i = decodeOne(s[i-1])*dp[1]
 *               + decodeTwo(s[i-2],s[i-1])*dp[0];
 *     dp_i %= 1e9+7;
 *     dp[0] = dp[1];
 *     dp[1] = dp_i;
 */
